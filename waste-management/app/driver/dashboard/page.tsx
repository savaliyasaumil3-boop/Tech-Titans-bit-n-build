...
export default function DriverDashboardPage() {
  ...
  const submitPickup = async () => {
    ...
    if (nextStop.bin_id !== binConfirmation.trim() || !Number.isFinite(collectedWeight) || collectedWeight <= 0 || collectedWeight > remainingCapacity || !Number.isFinite(residualFill) || residualFill < 0 || residualFill > 100) {
      setNotice("Confirm the bin ID and enter a valid weight within the truck's remaining capacity.");
      return;
    }
    setBusy(true); setSyncState("pending");
    ...
    if (!error) {
      const stopStatus = pickupOutcome === "partial" ? "partial" : "completed";
