...
export function SmartBinViewer() {
  ...
  const angles: Array<{ id: InspectionAngle; label: string; icon: string }> = [
    { id: "front", label: "Front View", icon: "👁️" },
    { id: "top", label: "Top Lid", icon: "🔝" },
    { id: "side", label: "Side View", icon: "📐" },
    { id: "inside", label: "Internal Fill", icon: "🔍" },
  ];
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md">
        <motion.div
          ...
          className="relative w-full max-w-6xl h-[90vh] max-h-[850px] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col text-slate-100"
        >
          {/* Header Bar */}
          <div className="h-16 px-6 border-b border-slate-800 bg-slate-950/80 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-bold tracking-tight">{bin.id} — 3D Digital Twin Inspection</h2>
                  <Badge
                    variant={
                ...
                <p className="text-xs text-slate-400">{bin.location_name} • Ahmedabad Municipal Corporation</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                size="icon"
                variant="ghost"
                className="h-9 w-9 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white"
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
            ...
            <div className="lg:col-span-8 relative flex flex-col p-4 bg-slate-950/60 border-r border-slate-800/80">
              {/* R3F 3D Canvas */}
              ...
              <div className="mt-4 p-3 rounded-xl bg-slate-900/90 border border-slate-800 backdrop-blur-md flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  {isLidOpen ? <Unlock className="h-3.5 w-3.5 text-yellow-400" /> : <Lock className="h-3.5 w-3.5" />}
                ...
                <div className="flex items-center gap-3 min-w-[200px] flex-1 max-w-[280px]">
                  <span className="text-xs text-slate-400 font-medium shrink-0">Simulate Fill:</span>
                  <Slider
                    value={[displayFill]}
                    min={0}
                    max={100}
                    step={1}
                    onValueChange={(val) => setSimulatedFill(val[0])}
                    className="flex-1"
                  />
            ...
            <div className="lg:col-span-4 p-5 space-y-5 overflow-y-auto bg-slate-900/40">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3.5 rounded-xl bg-slate-800/50 border border-slate-700/60">
                  ...
                  <p className="text-2xl font-bold tracking-tight text-slate-100">{displayFill}%</p>
                  <p className="text-[11px] text-slate-400">{displayKg} / {bin.capacity_kg} kg payload</p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-800/50 border border-slate-700/60">
                  ...
                  <p className="text-2xl font-bold tracking-tight text-slate-100">{bin.priority_score}</p>
                  <p className="text-[11px] text-slate-400">Dispatch Priority Index</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between py-1 border-b border-slate-800/60">
                    <span className="text-slate-400">Ultrasonic Clearance</span>
                  ...
                  <div className="flex justify-between py-1 border-b border-slate-800/60">
                    <span className="text-slate-400 flex items-center gap-1">
                      <Battery className="h-3 w-3 text-green-400" /> Battery Charge
                    </span>
                    <span className="font-mono font-semibold text-slate-200">94% (Solar Charged)</span>
                  </div>

                  <div className="flex justify-between py-1 border-b border-slate-800/60">
                    <span className="text-slate-400 flex items-center gap-1">
                      <Thermometer className="h-3 w-3 text-amber-400" /> Internal Temperature
                    </span>
                    <span className="font-mono font-semibold text-slate-200">31.4 °C</span>
                  </div>

                  <div className="flex justify-between py-1 border-b border-slate-800/60">
                    <span className="text-slate-400">Waste Material Stream</span>
                    <span className="font-medium text-primary">{bin.waste_type}</span>
                  </div>

                  <div className="flex justify-between py-1">
                    <span className="text-slate-400 flex items-center gap-1">
                      <Clock className="h-3 w-3 text-blue-400" /> Predicted Overflow
                    </span>
